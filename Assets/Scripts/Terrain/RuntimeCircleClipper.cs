...
public class RuntimeCircleClipper : MonoBehaviour, IClip
{
    ...
    public LayerMask destructableTerrain;
    public float distance2Terrain=20f;
    ...
    void UpdateTouch()
    {
        if (TouchUtility.TouchCount > 0)
        {
            ...
            if (touch.phase == TouchPhase.Began)
            {
                ...
                if (Physics.Raycast(ray, out hit, distance2Terrain, destructableTerrain))
                {
                    //Vector2 XOYPlaneLocation = mainCamera.ScreenToWorldPoint(new Vector3(touchPosition.x, touchPosition.y, -cameraZPos));
                    currentTouchPoint = new Vector2f(hit.point.x, hit.point.y) - terrain.GetPositionOffset();
